...
namespace Ship.Ses.Transmitter.WebApi.Controllers.v1
{
    ...
    public class FhirIngestController : ControllerBase
    {
        ...
        public async Task<ActionResult<FhirIngestAcceptedResponse>> Post([FromBody] FhirIngestRequest request)
        {
            ...
            if (string.IsNullOrWhiteSpace(request.ShipService))
                return Problem(title: "Bad request", detail: "Missing required field: ShipService.",
                               statusCode: StatusCodes.Status400BadRequest);
            ...
            try
            {
                ...
                switch (result.Outcome)
                {
                    case IdempotentInsertOutcome.Inserted:
                        _logger.LogInformation("✅ Ingested corr={Corr} client={Client} facility={Facility}",
            ...
            {
                _logger.LogError(ex, "🔥 Unexpected error ingesting {Id}.", resourceId ?? "(none)");
